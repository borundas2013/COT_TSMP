...
def create_group_relationship_model(pretrained_model, max_length, vocab_size):
    ...
    inputs = [
        ...
        tf.keras.layers.Input(shape=(1,), name='er_list'),
        tf.keras.layers.Input(shape=(1,), name='tg_list')
    ]
    ...
    def property_branch():
        return tf.keras.Sequential([
            ...
            tf.keras.layers.LayerNormalization(),
            tf.keras.layers.Dropout(0.1),
            # Add layers to match dimensions
            tf.keras.layers.Dense(256, activation='relu'),
            # Reshape to match group_aware features dimensions
            tf.keras.layers.Lambda(lambda x: tf.expand_dims(x, 1)),
            tf.keras.layers.Lambda(lambda x: tf.tile(x, [1, 245, 1]))  # 245 is max_length
        ])
    
    relationship_block = create_relationship_block()
    property_processor = property_branch()

    property_features = tf.keras.layers.Concatenate()([er_list, tg_list])
    processed_properties = property_processor(property_features)
    
    # Process pairs with attention to their relationship
    combined_features = tf.keras.layers.Concatenate()(
        [group_aware1, group_aware2, processed_properties]
    )
