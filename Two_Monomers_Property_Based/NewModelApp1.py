...
from tensorflow.keras.layers import Input,  Dense, Add, Dropout, Concatenate, Lambda
import Constants
...
def create_model(max_length, vocab_size, pretrained_model):
    inputs = [
        ...
        Input(shape=(1,), name='er_list'),
        Input(shape=(1,), name='tg_list')
    ]
    
    monomer1_input, monomer2_input, group_input, decoder_input1, decoder_input2, er_list, tg_list = inputs
    ...
    group_projected = Dense(128, activation="relu", name="group_projection")(group_input)

    er_projected = Dense(128, activation="relu", name="er_projection")(er_list)
    tg_projected = Dense(128, activation="relu", name="tg_projection")(tg_list)
    property_context = Concatenate()([er_projected, tg_projected])
    property_context = Dense(128, activation="relu")(property_context)

    # combined_input = Concatenate(name="combined_input")([group_projected, er_projected, tg_projected])
    # combined_projection = Dense(128, activation="relu", name="combined_projection")(combined_input)

   

    # ✅ Combine encoder state + group state using addition
    modified_state1 = Add(name="modified_state1")([encoder_state1_projected, group_projected,property_context])
    modified_state2 = Add(name="modified_state2")([encoder_state2_projected, group_projected,property_context])
    ...
    outputs = {
    'monomer1_output': monomer1_output,
    'monomer2_output': monomer2_output
    }

    two_monomer_model = Model(inputs=inputs, outputs=outputs)
