...
import Two_Monomers_Property_Based.RLHF.RLHFConstants as RLHFConstants
...
class PPOTrainer:
    def __init__(
        ...
        self.property_reward = property_reward
    ...
    def collect_samples(self, input_data_batch):
        ...
        er_list = tf.reshape(input_data_batch['er_list'], (-1, 1))
        tg_list = tf.reshape(input_data_batch['tg_list'], (-1, 1))
        attempts = 0
        sample=0
        ...
        while sample < RLHFConstants.NUM_SAMPLES:
            ...
            generated_tokens_1 = generated_tokens['monomer1_output']
            generated_tokens_2 = generated_tokens['monomer2_output']
    ...
    def train_step_single_device(self, input_data,epoch,num_devices):
        ...
        with tf.GradientTape() as tape:
            ...
            property_rewards = self.property_reward.calculate_batch_reward(samples)
            total_score,er_score,tg_score = property_rewards['total_score'],property_rewards['er_score'],property_rewards['tg_score']
            
            rewards = tf.convert_to_tensor(rewards, dtype=tf.float32)
            diversity_rewards = tf.convert_to_tensor(diversity_rewards, dtype=tf.float32) 
            property_rewards = tf.convert_to_tensor(total_score, dtype=tf.float32)
            
            
            total_rewards = 0.5 * rewards + 0.25 * diversity_rewards + 0.25 * property_rewards
            total_rewards = (total_rewards - tf.reduce_mean(total_rewards)) / (tf.math.reduce_std(total_rewards) + 1e-8)
            print("Total rewards:", total_rewards, tf.reduce_mean(total_rewards))
...
if __name__ == "__main__":
    ...
    property_reward = PropertyRewardCalculator()
    ...
    trainer = PPOTrainer(
        ...
        property_reward=property_reward,
    ...
    valid_dataset = tf.data.Dataset.from_tensor_slices((
        {
            ...
            'er_list': test_x['er_list'],
            'tg_list': test_x['tg_list']
        },
        {
