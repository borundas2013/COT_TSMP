...
class GeneratorModel(tf.keras.Model):
    ...
    def compile_model(self):
        """Compile the prediction model with CustomMoleculeLoss for RLHF training"""
        from CustomLoss import CustomMoleculeLoss
        
        # Create reverse vocab mapping
        reverse_vocab = {idx: token for token, idx in self.vocab.items()}
        
        # Initialize the custom loss
        custom_loss = CustomMoleculeLoss(
            group_smarts_list=self.group_combinations,
    ...
    def generate(self, input_data_batch,training=True):
        ...
        predictions = self.prediction_model({
            ...
            'er_list': input_data_batch['er_list'],
            'tg_list': input_data_batch['tg_list']
        }, training=training)
    ...
    def get_action_distribution(self, samples):
        ...
        for sample in samples:
            try:
                ...
                er_list = np.array([sample['actual_er']])
                tg_list = np.array([sample['actual_tg']])
                ...
                action_distribution1 = output['monomer1_output'][0]
                action_distribution2 = output['monomer2_output'][0]  # Second monomer
...
if __name__ == "__main__":
    save_dir_abs = os.path.join("Two_Monomers_Property_Based", "pretrained_model", "saved_models_rl_gpu_3")
    file_path = os.path.join('Two_Monomers_Property_Based', 'Dataset', Constants.DATA_PATH)
    ...
    generator.prediction_model.load_weights(os.path.join(root_dir, "RLHF","Generator","models","property_based_rl_model", "weights_model.weights.h5"))
    smiles1 = "C1=CC=CC=C1"
    smiles2 = "C1=CC=CC=C1"
    group1 = "C=C"
    group2 = "C=C"
