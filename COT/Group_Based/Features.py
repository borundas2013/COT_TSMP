...
def data_filterize(df):
    return df[df['Smiles'].apply(lambda x: len(x.split(',')) == 2)]
...
def create_mol_from_graph(adjacency, features):
    ...
    keep_idx = tf.where(tf.argmax(features, axis=1) != Constants.ATOM_DIM - 1)
    keep_idx = tf.squeeze(keep_idx)  # Remove extra dimensions
    
    # Use tf.gather to index the tensors
    features = tf.gather(features, keep_idx)
    adjacency = tf.gather(tf.gather(adjacency, keep_idx, axis=0), keep_idx, axis=1)
    
    # Convert back to numpy for RDKit operations
...
def prepare_graph_features(df_smiles):
    ...
    for idx,rows in df_smiles.iterrows():
        row_smiles=rows['Smiles'][1:-1]
        row_smiles=row_smiles.replace("'","")
        smiles=row_smiles.split(',')
...
def get_condtions(df_smiles):
    ...
    for idx, rows in df_smiles.iterrows():
        ...
        row_smiles = row_smiles.replace("'", "")
        smiles = row_smiles.split(',')
        imine_features_cond_1, imine_features_cond_2 = prepare_condition(smiles)
...
def get_property_matrix(df_smiles):
