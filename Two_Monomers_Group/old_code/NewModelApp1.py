...
def beam_search(model, input_data, vocab, k=5, max_length=100):
    print("Beam search started")
    start_token = vocab["<start>"]
    end_token = vocab["<end>"]

    # Initialize sequences with the start token
    sequences = [((tf.constant([[start_token]]), tf.constant([[start_token]])), 0)]  

    for _ in range(max_length):
        all_candidates = []

        for (seq1, seq2), score in sequences:
            # ✅ Predict next tokens (unpack both outputs)
            prediction1, prediction2 = model.predict(input_data)  

            # ✅ Convert to numpy array if necessary
            prediction1 = np.array(prediction1)
            prediction2 = np.array(prediction2)

            # ✅ Take log probabilities for more stable scores
            log_probs1 = np.log(prediction1[0, -1, :])
            log_probs2 = np.log(prediction2[0, -1, :])

            # ✅ Get top k predictions for each output
            top_k_indices1 = np.argsort(log_probs1)[-k:]
            top_k_indices2 = np.argsort(log_probs2)[-k:]

            for i in top_k_indices1:
                for j in top_k_indices2:
                    # ✅ Stop generation if the end token is predicted
                    if i == end_token and j == end_token:
                        all_candidates.append(((seq1, seq2), score))
                        continue

                    candidate1 = tf.concat([seq1, [[i]]], axis=-1)
                    candidate2 = tf.concat([seq2, [[j]]], axis=-1)
                    candidate_score = score + log_probs1[i] + log_probs2[j]  # Combine scores
                    all_candidates.append(((candidate1, candidate2), candidate_score))

        # ✅ Keep top k sequences based on combined score
        sequences = sorted(all_candidates, key=lambda x: x[1], reverse=True)[:k]

        # ✅ Stop if all sequences ended with the end token
        if all(seq1[:, -1] == end_token and seq2[:, -1] == end_token for (seq1, seq2), _ in sequences):
            break

    # ✅ Best sequence = highest combined score
    best_seq1 = sequences[0][0][0].numpy().flatten()
    best_seq2 = sequences[0][0][1].numpy().flatten()

    return [decode_smiles(best_seq1), decode_smiles(best_seq2)]
