...
from diffusion.molecule_diffusion import *
...
def generate_complete_smiles(model, x_batch):
    """Generate complete SMILES sequences using diffusion model"""
    # Model expects the data in a specific format
    outputs = model.train_step(x_batch)
    
    # Return the generated tokens
    return outputs['pred_monomer1'], outputs['pred_monomer2']

def custom_training_loop(model, train_data, val_data=None, epochs=1, max_length=245, vocab_size=None, vocab=None):
    ...
    new_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4), loss=custom_loss)
    ...
    try:
        ...
        for x_batch, y_batch in val_dataset:
            try:
                ...
                generated_smiles1, generated_smiles2 = generate_complete_smiles(
                    model, x_batch, max_length=max_length,vocab=vocab
                )
                original_monomer1 = x_batch['original_monomer1']
                original_monomer2 = x_batch['original_monomer2']
                print("Original Monomer 1: ", original_monomer1.shape)
...
if __name__ == "__main__":
    ...
    try:
        ...
        #     )
        custom_loss = CombinedLoss()
        new_model =  build_diffusion_model(
            pretrained_model=pretrained_model,
            num_steps=1000,
            vocab_size=len(smiles_vocab),
            embedding_dim=128,
