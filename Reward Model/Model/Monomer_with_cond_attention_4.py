...
s = df_smiles[10]  # ["C=CC(Cl)=O","NCC(=O)O"]
print(s)
...
def checkFunctionalGroups(smile1, smile2):
    ...
    if result_epoxy[0] and result_imine_1[0]:
        return (result_epoxy[2],result_imine_1[2], EPOXY_GROUP, False)
    elif result_imine[0] and result_epoxy_1[0]:
        return  (result_imine[2], result_epoxy_1[2], IMINE_GROUP, result_imine[3])
    elif result_vinyl[0] and result_vinyl_1[0]:
        return (result_vinyl[2], result_vinyl_1[2], VINYL_GROUP, False)
    elif result_vinyl[0] and result_thiol_1[0]:
        return (result_vinyl[2], result_thiol_1[2], THIOL_GROUP, False)
    elif result_thiol[0] and result_vinyl_1[0]:
        return (result_thiol[2], result_vinyl_1[2], ACRYLATE_GROUP, False)
    else: 
        return None
     
def make_condtion(indices_1, indices_2, condtion, is_capital_C,imine_features_cond_1,imine_features_cond_2):
    if condtion == EPOXY_GROUP:
    ...
    if condtion == IMINE_GROUP:
        imine_features_cond_1=indexize(indices_1,imine_features_cond_1,1)
        imine_features_cond_2=indexize(indices_2,imine_features_cond_2,1)
    if condtion == VINYL_GROUP:
    ...
    if condtion == THIOL_GROUP:
        imine_features_cond_1=indexize(indices_1,imine_features_cond_1,2)
        imine_features_cond_2=indexize(indices_2,imine_features_cond_2,1)
    if condtion == ACRYLATE_GROUP:
        imine_features_cond_1=indexize(indices_1,imine_features_cond_1,1)
        imine_features_cond_2=indexize(indices_2,imine_features_cond_2,2)
...
def prepare_condition(sample):
    ...
    imine_features_cond_1 = np.zeros((NUM_ATOMS, NUM_ATOMS), 'float32')
    imine_features_cond_2 = np.zeros((NUM_ATOMS, NUM_ATOMS), 'float32')
...
#     mol =molecule # Chem.MolFromSmiles(molecule)
#     if mol is None:
#         return None  # Invalid molecule
    
   
#     benzene_rings = Chem.GetSymmSSSR(mol)
#     for ring in benzene_rings:
#         # Count double bonds in the ring
#         double_bonds = sum(1 for atom_idx in ring for bond in mol.GetAtomWithIdx(atom_idx).GetBonds() if bond.GetBondType() == Chem.rdchem.BondType.DOUBLE)
#         if double_bonds > 4:
#             return True
#         return False
...
def graph_to_molecule2_d1(graphs):
    ...
    for i in range(len(adjecenies_array_0)):
        ...
        features_0 = tf.one_hot(features_0, depth=ATOM_DIM, axis=1)
        ...
        features_1 = tf.one_hot(features_1, depth=ATOM_DIM, axis=1)
        #print(adjacency_0.shape,features_0.shape)
        
        # adjacency_0, features_0, adjacency_1, features_1 = adjecenies_array_0[i], features_array_0[i], \
        #                                                    adjecenies_array_1[i], features_array_1[i]
        mols = [formMol(adjacency_0, features_0.numpy()), formMol(adjacency_1, features_1.numpy())]
        all_mols.append(mols)
...
class RelationalGraphConvLayer(keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        x=x1 = keras.layers.Concatenate(axis=-1)([x1,conditional_feature_1]) #tf.add(x1,conditional_features_transposed)#
        ...
        y=x2 =  keras.layers.Concatenate(axis=-1)([x2,conditional_feature_2])#tf.add(x2,conditional_features_transposed_2)#
...
def get_encoder(gconv_units, latent_dim, adjacency_shape, feature_shape, imine_shape, condition_shape,
    ...
    encoder = keras.Model([adjacency_0, features_0, adjacency_1, features_1,
                          ...
                          [z_mean, log_var, z_mean_1, log_var_1,conditional_features_1,conditional_features_2], name='encoder')
...
def get_decoder(dense_units, dropout_rate, latent_dim, adjacency_shape, feature_shape, condition_shape):
   ...
   # x=x+y
    ...
    decoder = keras.Model(
        [latent_inputs, latent_inputs_1,conditional_features_1,conditional_features_2], outputs=[x_0_adjacency, x_0_features, x_1_adjacency, x_1_features],
        name="decoder"
    )
...
class MoleculeGenerator(keras.Model):
    ...
    def _compute_loss(self, z_log_var, z_mean, z_mean_1, z_log_var_1, graph_real, graph_generated, imine_tensor,
        ...
        total_loss=loss_BenzeneRing(adjacency_0_gen, features_0_gen, adjacency_1_gen, features_1_gen,total_loss)
...
def write_samples_in_file():
    mols = []
    i=0
    with open('saved/test.txt', 'a') as the_file:
        # random.seed(121)
        smiles = df_smiles#random.choices(df_smiles, k=25)
