...
def make_condition(array_data, type_cond, start_index, condition_indicies, smile, isCapital_C):
    ...
    if type_cond == IMINE_GROUP_VALUE:
        ...
        array_data[0][idx1] = 1  # char_to_index['N']
        if isCapital_C:
            array_data[0][idx2] = 1  # char_to_index['C']
        else:
            array_data[0][idx2] = 1  # char_to_index['c']
            
    if type_cond == DOUBLE_C_GROUP:
        idx1, idx2 = condition_indicies
        array_data[0][idx1] = 1  # char_to_index["C"]
        array_data[0][idx2] = 1  # char_to_index["="]
        array_data[0][idx2 + 1] = 1  # char_to_index["C"]
    return array_data


def make_condition_1(array_data, type_cond, start_index, condition_indicies,smile, isCapital_C):
    ...
    if type_cond == IMINE_GROUP_VALUE:
        ...
        #cond="Nc"
        idx1,idx2=condition_indicies
        array_data[0][idx1]= char_to_index['N'] 
        if isCapital_C:
            array_data[0][idx2]= char_to_index['C']  
        else:
            array_data[0][idx2]= char_to_index['c']  
        
    if type_cond == DOUBLE_C_GROUP:
        idx1,idx2=condition_indicies
        array_data[0][idx1]= char_to_index["C"]
        array_data[0][idx2]= char_to_index["="]
        array_data[0][idx2+1]= char_to_index["C"]
    return array_data
...
class RelationalGraphConvLayer(keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        x=x1 = keras.layers.Concatenate(axis=-1)([x1,conditional_features_transposed,imine_feature]) #tf.add(x1,conditional_features_transposed)#
        ...
        y=x2 =  keras.layers.Concatenate(axis=-1)([x2,conditional_features_transposed_2,imine_feature])#tf.add(x2,conditional_features_transposed_2)#
...
class MoleculeGenerator(keras.Model):
    ...
    def _compute_loss(self, z_log_var, z_mean, z_mean_1, z_log_var_1, graph_real, graph_generated, imine_tensor,
        ...
        epochs_loss_dict.append(adjacency_0_loss + features_1_loss + kl_loss_0 + adjacency_1_loss + features_0_loss + kl_loss_1 )
...
def write_samples_in_file():
    mols = []
    i=0
    with open('/home/C00521897/Fall 22/New_Monomer_generation/Data/output/output_10085.txt', 'a') as the_file:
        # random.seed(121)
