...
def read_smiles_from_file(filename):
    try:
        ...
        # Extract SMILES column as a list and remove duplicates
        smiles_list = df['SMILES'].tolist()
        smiles_list = list(set(smiles_list))
        # np.random.shuffle(smiles_list)
        # df_unique = pd.DataFrame({'SMILES': smiles_list})
        # unique_filename = f'{get_project_root()}\Processed_Data\{filename.replace(".csv","")}_unique.csv'
        # df_unique.to_csv(unique_filename, index=False)
        
        print(f"Successfully loaded {len(smiles_list)} unique SMILES strings from combined dataset")
        return smiles_list
        
    except Exception as e:
        print(f"Error reading SMILES data: {e}")
        return None
...
def make_training_data(smiles_list, vocab_path, tokenizer_path):
...
#     filename = 'CombinedData_K_CH_DE_unique.csv'
#     vocab_path = 'Code/tokenizers/vocab_2000/vocab.txt'
#     tokenizer_path = 'Code/tokenizers/vocab_2000/tokenizer'
...
#         make_training_data(smiles_list,vocab_path,tokenizer_path)
#     print(smiles_list[5000])
#     tokens = extract_tokens(smiles_list[5000], tokenizer_path)
#     print(tokens)
#     decoded_smiles = decode_smiles(tokens, tokenizer_path)
